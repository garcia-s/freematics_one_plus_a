...
#endif
#define LINK_UART_BAUDRATE 115200L
...
public:
  bool begin(unsigned int baudrate = LINK_UART_BAUDRATE,
             int rxPin = PIN_LINK_UART_RX, int txPin = PIN_LINK_UART_TX);
  void end();
  // send command and receive response
  int sendCommand(const char *cmd, char *buf, int bufsize,
...
public:
  ...
  bool xbBegin(unsigned long baudrate = BEE_BAUDRATE,
               int pinRx = PIN_BEE_UART_RXD, int pinTx = PIN_BEE_UART_TXD);
  void xbEnd();
  ...
  int xbReceive(char *buffer, int bufsize, unsigned int timeout = 1000,
                const char **expected = 0, byte expectedCount = 0);
