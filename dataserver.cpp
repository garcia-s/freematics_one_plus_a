...
int handlerInfo(UrlHandlerParam *param) {
  ...
  int bytes = snprintf(buf, bufsize,
                       "{\"httpd\":{\"uptime\":%lu,\"clients\":%d,\"requests\":"
                       "%u,\"traffic\":%u},\n",
                       millis(), httpParam.stats.clientCount,
                       (unsigned int)httpParam.stats.reqCount,
                       (unsigned int)(httpParam.stats.totalSentBytes >> 10));
  ...
  if (timeinfo.tm_year) {
    bytes += snprintf(
        ...
        "\"rtc\":{\"date\":\"%04u-%02u-%02u\",\"time\":\"%02u:%02u:%02u\"},\n",
...
#else
  bytes += snprintf(buf + bytes, bufsize - bytes,
                    "\"sd\":{\"total\":%llu,\"used\":%llu}", SD.totalBytes(),
                    SD.usedBytes());
#endif
...
int handlerLogData(UrlHandlerParam *param) {
  ...
  if (ctx) {
    ...
    if (!ctx->file) {
      param->contentLength =
          sprintf(param->pucBuffer, "{\"error\":\"Data file not found\"}");
      delete ctx;
      return FLAG_DATA_RAW;
    }
    ...
    if (c == '\n') {
      ...
      if (value++) {
        ...
        if (pid == 0) {
          ...
          param->contentLength += snprintf(
              param->pucBuffer + param->contentLength,
              param->bufSize - param->contentLength, "[%u,%s],", ts, value);
        }
      }
      len = 0;
  ...
  if (root) {
    while (file = root.openNextFile()) {
      ...
      if (!strncmp(fn, "/DATA/", 6)) {
        ...
        if (id) {
          n += snprintf(buf + n, bufsize - n, "{\"id\":%u,\"size\":%u", id,
                        size);
          if (id == fileid) {
...
UrlHandler urlHandlerList[] = {{"api/live", handlerLiveData},
