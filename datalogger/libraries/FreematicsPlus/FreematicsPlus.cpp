...
static const uint8_t gpsSettings[] = {0xB5, 0x62, 0x06, 0x8A, 0x13, 0x00, 0x01,
                                      0x01, 0x00, 0x00, 0xBB, 0x00, 0x91, 0x20,
                                      0x02, 0xAC, 0x00, 0x91, 0x20, 0x0A, 0x07,
                                      0x00, 0x91, 0x20, 0x00, 0x32, 0x74};
...
static void gps_soft_decode_task(void *inst) {
  ...
  for (;;) {
    ...
    duration =
        (uint32_t)9 * F_CPU / GPS_SOFT_BAUDRATE + F_CPU / GPS_SOFT_BAUDRATE / 2;
    do {
      taskYIELD();
    } while (getCycleCount() - start < duration);
  }
...
bool Task::create(void (*task)(void *), const char *name, int priority,
  ...
  BaseType_t xReturned =
      xTaskCreate(task, name, stacksize, (void *)this, priority, &xHandle);
...
void Task::sleep(uint32_t ms) { vTaskDelay(ms / portTICK_PERIOD_MS); }

bool Task::running() { return xHandle != 0; }
...
bool CLink_UART::begin(unsigned int baudrate, int rxPin, int txPin) {
  ...
  // Set UART pins
  uart_set_pin(LINK_UART_NUM, txPin, rxPin, UART_PIN_NO_CHANGE,
               UART_PIN_NO_CHANGE);
  // Install UART driver
  if (uart_driver_install(LINK_UART_NUM, LINK_UART_BUF_SIZE, 0, 0, NULL, 0) !=
      ESP_OK)
    return false;
  return true;
}
...
int CLink_UART::receive(char *buffer, int bufsize, unsigned int timeout) {
  ...
  for (;;) {
    ...
    if (n >= bufsize - 1)
      break;
    int len = uart_read_bytes(LINK_UART_NUM, (uint8_t *)buffer + n,
                              bufsize - n - 1, 1);
    if (len < 0)
      break;
    if (len == 0)
...
int CLink_UART::sendCommand(const char *cmd, char *buf, int bufsize,
...
int CLink_SPI::receive(char *buffer, int bufsize, unsigned int timeout) {
  ...
  if (eos)
    while (digitalRead(PIN_LINK_SPI_READY) == LOW)
      delay(1);
  return n;
}
...
int CLink_SPI::sendCommand(const char *cmd, char *buf, int bufsize,
                           unsigned int timeout) {
  uint32_t t = millis();
  int n = 0;
...
bool FreematicsESP32::gpsBeginExt(int baudrate) {
  ...
  pinMode(PIN_GPS_POWER2, OUTPUT);
  digitalWrite(PIN_GPS_POWER2, LOW);
  ...
  if (!(m_flags & FLAG_GNSS_SOFT_SERIAL)) {
    ...
    // // set UART pins
    // uart_set_pin(gpsUARTNum, pinGPSTx, pinGPSRx, UART_PIN_NO_CHANGE,
    //              UART_PIN_NO_CHANGE);
    // // install UART driver
    ...
    // set UART pins
    uart_set_pin(GPS_UART_NUM, PIN_GPS_UART_TXD2, PIN_GPS_UART_RXD2,
                 UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE);
    // install UART driver
    uart_driver_install(GPS_UART_NUM, UART_BUF_SIZE, 0, 0, NULL, 0);
    // turn on GPS power
    digitalWrite(PIN_GPS_POWER2, HIGH);
    delay(100);
    ...
    if (m_flags & FLAG_GNSS_SOFT_SERIAL) {
      ...
      const uint8_t packet[] = {0x0,  0x0,  0xB5, 0x62, 0x06, 0x8A, 0x0C, 0x00,
                                0x01, 0x01, 0x00, 0x00, 0x01, 0x00, 0x52, 0x40,
                                0x00, 0x96, 0x00, 0x00, 0xC7, 0x2B};
      // set GNSS baudrate to 38400bps for M8
      // const uint8_t packet[] = {0x0, 0x0, 0xB5, 0x62, 0x06, 0x0, 0x14, 0x0,
      // 0x01, 0x0, 0x0, 0x0, 0xD0, 0x08, 0x0, 0x0, 0x0, 0x96, 0x0, 0x0, 0x7,
      // 0x0, 0x3, 0x0, 0x0, 0x0, 0x0, 0x0, 0x93, 0x90};
      for (int i = 0; i < sizeof(packet); i++)
        softSerialTx(baudrate, packet[i]);
    }
#endif
  ...
  for (int i = 0; i < 60; i++) {
    ...
    if (s1 != s2) {
      ...
      if (m_flags & FLAG_GNSS_SOFT_SERIAL) {
        // apply GNSS settings
        for (int i = 0; i < sizeof(gpsSettings); i++)
          softSerialTx(GPS_SOFT_BAUDRATE, gpsSettings[i]);
      }
#endif
      return true;
    }
...
bool FreematicsESP32::gpsBegin() {
  ...
  Serial.println("Starting with gpsBegin");
  if (!link)
    return false;

  Serial.println("Link is not false");
  ...
  for (int i = 0; i < sizeof(gpsSettings); i++)
    n += sprintf(buf + n, "%02X ", gpsSettings[i]);
  buf[n - 1] = '\r';
...
bool FreematicsESP32::gpsGetData(GPS_DATA **pgd) {
  ...
  if (m_flags & FLAG_GNSS_USE_LINK) {
    ...
    if (gpsData.lat || gpsData.lng) {
      // filter out invalid coordinates
      good = (abs(lat - gpsData.lat * 1000000) < 100000 &&
              abs(lng - gpsData.lng * 1000000) < 100000);
    }
    if (!good)
      return false;
    gpsData.ts = millis();
    ...
    gps.get_datetime((unsigned long *)&gpsData.date,
                     (unsigned long *)&gpsData.time, 0);
    long alt = gps.altitude();
    if (alt != TinyGPS::GPS_INVALID_ALTITUDE)
      gpsData.alt = (float)alt / 100;
    unsigned long knot = gps.speed();
    if (knot != TinyGPS::GPS_INVALID_SPEED)
      gpsData.speed = (float)knot / 100;
    unsigned long course = gps.course();
    if (course < 36000)
      gpsData.heading = course / 100;
...
bool FreematicsESP32::xbBegin(unsigned long baudrate, int pinRx, int pinTx) {
  ...
  // Set UART pins
  uart_set_pin(BEE_UART_NUM, pinTx, pinRx, UART_PIN_NO_CHANGE,
               UART_PIN_NO_CHANGE);
  // Install UART driver
...
void FreematicsESP32::xbEnd() { uart_driver_delete(BEE_UART_NUM); }
...
int FreematicsESP32::xbRead(char *buffer, int bufsize, unsigned int timeout) {
  return uart_read_bytes(BEE_UART_NUM, buffer, bufsize,
                         timeout / portTICK_RATE_MS);
}

int FreematicsESP32::xbReceive(char *buffer, int bufsize, unsigned int timeout,
  ...
  do {
    ...
    if (n > 0) {
      ...
      for (byte i = 0; i < expectedCount; i++) {
        ...
        if (expected[i] && strstr(buffer, expected[i]))
          return i + 1;
      }
    } else if (n == -1) {
...
void FreematicsESP32::xbPurge() { uart_flush(BEE_UART_NUM); }
...
bool FreematicsESP32::begin(bool useCoProc, bool useCellular) {
  ...
  if (useCellular) {
    ...
    if (devType == 13) {
      pinRx = 32;
      pinTx = 33;
    } else if ((devType == 11 && !(m_flags & FLAG_USE_UART_LINK)) ||
               devType == 12 || devType == 0) {
#ifndef ARDUINO_ESP32C3_DEV
      // pinRx = 16;
      // pinTx = 17;
#endif
